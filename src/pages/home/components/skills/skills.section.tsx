...
function SkillsSection() {
	...
	const categories: SkillCategory[] = [
		{
			title: 'Programming Languages',
			skills: ['Python', 'JavaScript', 'Java', 'C++', 'Typescript', 'GoLang']
		},
		{
			title: 'Frameworks & Libraries',	
			skills: ['Node.js', 'React', 'Flutter', 'Ansible', 'TensorFlow', 'PyTorch', 'Angular', 'Express.js']
		...
		{
			title: 'Infrastructure',
			skills: ['ESXi Hypervisor', 'VMware','Baremetal', 'HANA DB', 'RHEL', 'SUSE Linux']
		},
		{
			title: 'Databases',
				...
				<div className={CN.generate('grid')}>
					{categories.map((category, index) => (
						<div key={index} className={CN.generate('category')}>
							<h3 className={CN.generate('category-title')}>{category.title}</h3>
							<div className={CN.generate('skills-list')}>
								{category.skills.map((skill, skillIndex) => (
									<span key={skillIndex} className={CN.generate('skill-tag')}>
										{skill}
									</span>
								))}
							</div>
